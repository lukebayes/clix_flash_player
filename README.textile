...
Finally, <code>open</code> has the additional disadvantage in that when we send SIGHUP to the parent process, the child Players are orphaned.

h3. <code>exec</code> Flash Player.app with the SWF file as an argument

Doesn't work because Flash Player.app is a directory. Thanks Steve!

h3. <code>exec</code> Flash Player.app/Contents/MacOS/Flash Player binary with the SWF file as an argument
...
Unfortunately, this doesn't work for us because the Flash Player is never focused, and actually launches _behind_ all other applications. When we attempt use AppleScript to focus the Player, this causes major issues if our SWF file has a runtime exception before the focus event was handled.
